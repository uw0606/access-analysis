...
export default function SurveyTable() {
  ...
  const processFile = async (file: File) => {
    ...
    // 表示名がTOUR等の場合を考慮した確定タイトル
    const confirmedTitle = (selectedLiveForImport.title === "TOUR" || !selectedLiveForImport.title) 
      ? (selectedLiveForImport.event_name || selectedLiveForImport.summary || "TOUR")
      : selectedLiveForImport.title;

    const targetDate = normalizeDate(selectedLiveForImport.event_date);
    const isAlreadyRegistered = registeredSet.has(`${targetDate}_${confirmedTitle}`);
    
    if (isAlreadyRegistered) {
      const confirmOverwrite = window.confirm(`「${confirmedTitle}」のデータは既に存在します。上書きしますか？`);
             ...
             <div className="bg-zinc-950 p-6 rounded-3xl border border-zinc-800">
              ...
              <div className="space-y-2 h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                {liveEvents.map(ev => {
                  ...
                  // タイトルの補正ロジック
                  const displayTitle = (ev.title === "TOUR" || !ev.title) 
                    ? (ev.event_name || ev.summary || "TOUR")
                    : ev.title;
                  
                  const isAlreadyRegistered = registeredSet.has(`${dateKey}_${displayTitle}`);
                  return (
                    <button key={ev.id} onClick={() => setSelectedLiveForImport(ev)} 
                      ...
                      <div className="font-bold text-[11px] mt-1 text-white">{displayTitle}</div>
                    </button>
                  );
                })}
              </div>
            ...
            <div className="space-y-6">
              {selectedLiveForImport && (
                <div className="bg-zinc-900/50 p-8 rounded-[40px] border border-red-600/30 space-y-8">
                  <h3 className="text-2xl font-black italic text-red-600 leading-none">
                    {(selectedLiveForImport.title === "TOUR" || !selectedLiveForImport.title) ? (selectedLiveForImport.event_name || selectedLiveForImport.summary || "TOUR") : selectedLiveForImport.title}
                  </h3>
