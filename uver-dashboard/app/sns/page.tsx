...
export default function SnsStats() {
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-6">
          ...
          {platform !== 'youtube' && (
            <div className="flex items-center gap-2 bg-zinc-950 p-1 rounded-lg border border-zinc-800">
              <input type="number" placeholder="New Count" className="bg-transparent border-none text-[11px] font-mono focus:outline-none w-28 px-3 py-1 text-white" value={inputValues[platform] || ""} onChange={(e) => setInputValues({ ...inputValues, [platform]: e.target.value })} />
              <button onClick={() => handleQuickSave(platform)} className="bg-white text-black text-[9px] font-black px-4 py-2 rounded hover:bg-red-600 hover:text-white transition-all uppercase">{saveStatus[platform] || "Update"}</button>
            </div>
          )}
        </div>
        ...
        <div className="h-[450px] w-full mb-6">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={platformData} margin={{ bottom: 0, top: 10 }}>
              ...
              <XAxis dataKey="date" stroke="#52525b" fontSize={10} tickLine={false} axisLine={false} dy={5} height={80} />
              <YAxis yAxisId="left" stroke="#52525b" fontSize={9} tickLine={false} axisLine={false} domain={['auto', 'auto']} />
              <YAxis yAxisId="right" orientation="right" hide={true} domain={[0, (max: number) => max * 3.5]} />
              ...
              <Tooltip 
                content={({ active, payload, label }) => {
                  ...
                  const dayEvents = events.filter(e => {
                    ...
                    return d.toLocaleDateString('ja-JP', { month: '2-digit', day: '2-digit' }) === label;
                  });
                  return (
                    <div className="bg-black/90 border border-zinc-800 p-3 rounded-lg text-[10px] shadow-2xl backdrop-blur-md">
                      ...
                      {payload.filter(p => p.name && p.name !== "Events").map((p: any) => (
              ...
              <Bar yAxisId="right" dataKey="diff" name="Daily Growth" fill={color} opacity={0.2} radius={[4, 4, 0, 0]} barSize={20} />
              ...
              <Line
                ...
                dot={(props) => {
                  ...
                  const dayEvents = events.filter(e => {
                    const d = new Date(e.event_date);
                    return d.toLocaleDateString('ja-JP', { month: '2-digit', day: '2-digit' }) === payload.date;
                  });
                  
                  const dotBaseY = 400; 
                  return (
                    <g key={`ev-sns-${payload.date}`}>
            ...
            <table className="w-full text-[10px] font-mono border-separate border-spacing-0">
              <thead className="sticky top-0 z-10 bg-zinc-950 shadow-md">
                <tr className="text-zinc-500 uppercase">
                  <th className="p-3 text-left border-b border-zinc-800 bg-zinc-950">Date</th>
                  <th className="p-3 text-right border-b border-zinc-800 bg-zinc-950">Total</th>
                  <th className="p-3 text-right border-b border-zinc-800 bg-zinc-950">Growth</th>
                  <th className="p-3 text-center w-16 border-b border-zinc-800 bg-zinc-950">Action</th>
                </tr>
              </thead>
              <tbody>
                {platformData.slice().reverse().map((item) => (
                  <tr key={item.id} className="hover:bg-white/5 transition-colors">
                    <td className="p-3 text-zinc-400 font-bold border-b border-zinc-800/30">{item.date}</td>
                    ...
                    <td className="p-3 text-center border-b border-zinc-800/30">
                      <button onClick={() => handleDelete(item.id)} className="text-zinc-700 hover:text-red-500 transition-colors uppercase text-[8px] font-black tracking-tighter">Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
  ...
  if (loading) return <div className="bg-black text-white min-h-screen flex items-center justify-center font-mono animate-pulse tracking-widest text-xs">SYNCHRONIZING SNS DATA...</div>;
  ...
  return (
    <main className="min-h-screen bg-black text-white p-4 md:p-12 relative">
      {selectedEvent && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-zinc-900 border border-zinc-700 w-full max-w-sm rounded-3xl p-8 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className={`inline-block px-3 py-1 rounded-full text-[8px] font-black mb-4 ${
              selectedEvent.category === 'LIVE' ? 'bg-red-600 text-white' : 
              selectedEvent.category === 'RELEASE' ? 'bg-yellow-500 text-black' :
              selectedEvent.category === 'TV' ? 'bg-emerald-500 text-white' :
              selectedEvent.category === 'OTHER' ? 'bg-blue-600 text-white' : 'bg-zinc-700 text-zinc-300'
            }`}>
      ...
      <header className="mb-12 max-w-5xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
        <div>
          <h1 className="text-3xl font-black italic uppercase tracking-tighter">SNS <span className="text-red-600">Analytics</span></h1>
          <p className="text-zinc-500 text-[9px] mt-1 uppercase tracking-[0.3em]">UVERworld Social Dominance Tracker</p>
        </div>
        <div className="flex gap-4">
          <a href="/" className="text-[10px] border border-zinc-700 px-6 py-2 rounded-full hover:bg-white hover:text-black transition-all font-bold uppercase tracking-widest">← Video Stats</a>
        </div>
      </header>
